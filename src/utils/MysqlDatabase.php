 ...
 *
 * @version      SVN: $Id$
 * @filesource   $HeadURL: https://encarnium.googlecode.com/svn/trunk/includes/classes/MysqlDatabase.php $
 * @revision     $Revision: 245 $
 * @modifiedby   $Author: Domac.Andreij $
 * @lastmodified $Date: 2010-07-07 11:47:59 +0200 (Mi, 07 Jul 2010) $
 */
...
class MysqlDatabase {
  ...
  public function connect() {
    if($this->active) {
      if($this->debug) {
        Logger::debug("connect :: connect to DataBase", "db");
      }
      ...
      if(!$this->dbObj) {
        ...
        if($this->debug) {
          Logger::warning("Verbindung zum Datenbank-Server konnte nicht hergestellt werden", "db");
        }
      }
      else {
        ...
        if(!mysql_select_db($this->db)) {
          ...
          Logger::debug("Verbindung zur Datenbank '{$this->db}' aufgebaut", "db");
  ...
  public function query($query) {
    if($this->active === "connect") {
      ...
      Logger::warning("Es wurde nicht mit der Datenbank verbunden","db");
      return false;
    }
  }
  ...
  public function select($statement) {
    if($this->active === "connect") {
      ...
      if($this->debug) Logger::debug("getData() :: Statement ist: {$statement}", "db");
      ...
      // die() wird in methoden nicht benutzt
          // Absenden
          if(!$query){
              if($this->debug) Logger::debug("setData() :: Query failed: ".mysql_error(), "db");
              return array();
          }
  ...
  public function update($table, array $fields, array $values, $condition = '') {
    if($this->active === "connect") {
      ...
      // Wenn debug Modus an ist
      if($this->debug) Logger::debug("changeData :: Statement ist: ".$statement, "db");
  ...
  public function insert($table, array $fields, array $values) {
    if($this->active === "connect") {
      ...
      // Wenn debug Modus an ist
      if($this->debug) Logger::debug("setData() :: Statement ist: ".$statement, "db");
  ...
  public function insertMore($table, array $fields, array $values) {
    if($this->active === "connect") {
      ...
      // Wenn debug Modus an ist
      if($this->debug) Logger::debug("setMoreData() :: Statement ist: ".$statement, "db");
  ...
  public function delete($table, $condition) {
   	if($this->active === "connect") {
   	  $statement = "DELETE FROM {$table} WHERE ".$condition;
          ...
          //Wenn debug Modus an ist
          if($this->debug) Logger::debug("deleteData :: Statement ist: ".$statement, "db");
